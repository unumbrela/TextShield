...
def pad_image(img, pad_value=0):
    """将单张图片 pad 到 28 的倍数, 返回 (padded_img, orig_h, orig_w)"""
    h, w = img.shape[:2]
...
def process_single(model, processor, device, orig_path, fname, output_dir,
                   max_new_tokens, save_padded=False):
    """处理单张图片: pad → 推理 → 解析 → 生成输出"""
    import tempfile
    ...
    # Pad 到 28 倍数
    padded_img, _, _ = pad_image(orig_img)

    # 保存 padded 图片到临时文件 (模型需要读文件路径)
    pad_h, pad_w = padded_img.shape[:2]
    tmp_dir = os.path.join(output_dir, '.tmp')
    os.makedirs(tmp_dir, exist_ok=True)
    padded_path = os.path.join(tmp_dir, fname)
    cv2.imwrite(padded_path, padded_img)

    # 推理
    response = run_inference(model, processor, device, padded_path, max_new_tokens)
